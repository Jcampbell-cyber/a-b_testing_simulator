...
export function EffectDetectionCalculator({ onNavigate }: EffectDetectionCalculatorProps) {
  ...
  const calculateMDE = () => {
    ...
    let variance: number;
    let effectSizeCohen: number;

    if (metricType === 'continuous') {
      variance = 2;
      effectSizeCohen = Math.sqrt((variance * Math.pow(zAlpha + zBeta, 2)) / totalN);
    } else {
      variance = 2 * proportion * (1 - proportion);
      effectSizeCohen = Math.sqrt((variance * Math.pow(zAlpha + zBeta, 2)) / totalN);
    }

    const absoluteMde = metricType === 'continuous' ? effectSizeCohen * stdev : effectSizeCohen;
    ...
    const absoluteMdeHalf = metricType === 'continuous' ? mdeHalf * stdev : mdeHalf;
    const relativeMdeHalf = metricType === 'continuous'
      ? (absoluteMdeHalf / mean) * 100
      : (absoluteMdeHalf / proportion) * 100;

    const absoluteMdeDouble = metricType === 'continuous' ? mdeDouble * stdev : mdeDouble;
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {metricType === 'continuous' && (
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="number" value={mean} onChange={(e) => setMean(parseFloat(e.target.value) || 0)} className="w-full bg-gray-700 text-white px-3 py-2 rounded" />
                  </div>
                  <div>
                    <label className="block text-white font-semibold mb-2">Standard Deviation</label>
                    <input type="number" min="0.01" value={stdev} onChange={(e) => setStdev(Math.max(0.01, parseFloat(e.target.value) || 1))} className="w-full bg-gray-700 text-white px-3 py-2 rounded" />
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" checked={sampleSizeMode === 'per-group'} onChange={() => setSampleSizeMode('per-group')} className="w-4 h-4" />
                      <span className="text-gray-300">Per Group</span>
                    </label>
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="radio" checked={sampleSizeMode === 'both'} onChange={() => setSampleSizeMode('both')} className="w-4 h-4" />
                ...
                <div>
                  ...
                  <input
                    type="number" min="1" value={sampleSizePerGroup}
                ...
                <div>
                  ...
                  {comparisonType !== 'none' && (
                    <div className="flex items-center gap-2 mt-2">
                      ...
                      <Link to="/fwer" className="flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs transition-colors">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-600 rounded p-4">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p className="text-2xl font-bold text-blue-400">±{result.relativeMde.toFixed(2)}%</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold text-blue-300">+{result.relativeMde.toFixed(2)}% relative</p>
                      <p className="text-xs text-gray-200 mt-2">
                        {metricType === 'continuous' ? `+${result.absoluteMde.toFixed(2)} absolute` : `+${(result.absoluteMde * 100).toFixed(2)} pp`}
                      </p>
                ...
                <div className="border-t border-gray-600 pt-4">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs text-gray-400 mb-1">MDE Relative</p>
                      <p className="text-2xl font-bold text-gray-300">±{result.relativeMdeHalf.toFixed(2)}%</p>
                      <p className="text-xs text-gray-400 mt-2">MDE Absolute</p>
                    ...
                    <div className="text-right">
                      <p className="text-xs text-gray-400 mb-1">Effect Detection</p>
                      <p className="text-lg font-bold text-gray-200">+{result.relativeMdeHalf.toFixed(2)}% relative</p>
                      <p className="text-xs text-gray-200 mt-2">
                        {metricType === 'continuous' ? `+${result.absoluteMdeHalf.toFixed(2)} absolute` : `+${(result.absoluteMdeHalf * 100).toFixed(2)} pp`}
                      </p>
                      <p className="text-xs text-gray-400 mt-2">Target Value</p>
                ...
                <div className="border-t border-gray-600 pt-4">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <p className="text-xs text-gray-400 mb-1">MDE Relative</p>
                      <p className="text-2xl font-bold text-green-400">±{result.relativeMdeDouble.toFixed(2)}%</p>
                      <p className="text-xs text-gray-400 mt-2">MDE Absolute</p>
                    ...
                    <div className="text-right">
                      <p className="text-xs text-gray-400 mb-1">Effect Detection</p>
                      <p className="text-lg font-bold text-green-300">+{result.relativeMdeDouble.toFixed(2)}% relative</p>
                      <p className="text-xs text-gray-200 mt-2">
                        {metricType === 'continuous' ? `+${result.absoluteMdeDouble.toFixed(2)} absolute` : `+${(result.absoluteMdeDouble * 100).toFixed(2)} pp`}
                      </p>
                      <p className="text-xs text-gray-400 mt-2">Target Value</p>
                    ...
                    {comparisonType !== 'none' && (
                      <li>
                        {comparisonType === 'control'
                          ...
                          : `${(numFlights * (numFlights - 1)) / 2} pairwise comparisons`} (Bonferroni)
                      </li>
                    )}
                  </ul>
                  {comparisonType !== 'none' && (
                    <Link to="/fwer" className="flex items-center gap-1 text-blue-400 hover:text-blue-300 text-xs transition-colors mt-2">
                      <Info className="w-3 h-3" />
                      <span>Learn more about FWER</span>
                    </Link>
                  )}
            ...
            <div className="space-y-6 text-gray-300">
              ...
              <div>
                ...
                <ul className="space-y-3 ml-4">
                  <li className="text-sm"><strong>Current Configuration:</strong> The MDE at your specified sample size, power, and significance level.</li>
                  <li className="text-sm"><strong>Half Sample Size:</strong> What you could detect with half the samples. Effects need to be larger when you collect fewer samples.</li>
                  <li className="text-sm"><strong>Double Sample Size:</strong> What you could detect with double the samples. Larger samples allow you to detect smaller, more subtle effects.</li>
                </ul>
              </div>
              <div>
                <h3 className="text-lg font-semibold text-emerald-400 mb-2">Sample Size Impact</h3>
                <p className="text-sm mb-2">MDE has an inverse square root relationship with sample size. This means:</p>
              ...
              <div>
                ...
                <p className="text-sm mb-2">When testing multiple flights, Bonferroni correction makes detection harder by requiring a stricter significance threshold:</p>
                <div className="bg-gray-900 rounded p-4 font-mono text-sm mb-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">Use this calculator to understand trade-offs. With a fixed sample size, you can either:</p>
